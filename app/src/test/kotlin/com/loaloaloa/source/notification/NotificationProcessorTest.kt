...
class NotificationProcessorTest {
    ...
    @Test fun `custom app maps to its display name`() {
        val result = processor.process(
            ...
            customApps = mapOf("com.example.shopapp" to "Shop App"),
        )!!
        assertThat(result.appId).isEqualTo("com.example.shopapp")
        assertThat(result.bankName).isEqualTo("Shop App")
        assertThat(result.amount).isEqualTo(250_000)
        assertThat(result.isIncome).isTrue()
    }

    @Test fun `unknown app without custom mapping returns null`() {
        val result = processor.process(
            packageName = "com.example.shopapp",
            title = "Shop App",
            text = "+250.000đ",
