...
class HistoryViewModelTest {
    ...
    ) = HistoryViewModel(repo, settings, clock, zone)
    ...
    @Test fun `non-spoke device can delete`() = runTest {
        val state = vm(seededRepo()).uiState.first { !it.isEmpty }
        assertThat(state.canDelete).isTrue()
    }

    @Test fun `employee spoke device cannot delete`() = runTest {
        val settings = FakeUserSettingsRepository(UserSettings(relayRole = RelayRole.SPOKE))
        val state = vm(seededRepo(), settings).uiState.first { !it.isEmpty }
